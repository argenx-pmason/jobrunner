...
export const submitJob = (api, jobPath, token, repo = "repository") => {
  ...
  return fetch(api + `/jobs/${repo}` + jobPath + "?action=run", requestOptions)
    .then((response) => {
...
export const getFileContents = async (api, token, file, repo = "repository") => {
  const apiRequest = `/${repo}/files/${file}?component=contents`,
    myHeaders = new Headers();
  myHeaders.append("X-Auth-Token", token);
  const requestOptions = {
...
export const getFileVersions = async (api, token, file, repo = "repository") => {
...
export const getChildren = async (
  ...
  const apiRequest = `/${repo}/containers/${path}?component=children&childType=all&limit=1000`,
    myHeaders = new Headers();
  myHeaders.append("X-Auth-Token", token);
  const requestOptions = {
    method: "GET",
